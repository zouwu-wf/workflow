...
export function generateConfigTemplate(format: 'ts' | 'js' | 'json' = 'ts'): string {
    ...
    if (format === 'json') {
        return generateJsonConfig(projectName, packageManager, workspace, hasChangeset, hasTurbo, gitBranch);
    }

    if (format === 'js') {
        return generateJsConfig(projectName, packageManager, workspace, hasChangeset, hasTurbo, gitBranch, buildSteps);
    }

    return generateTsConfig(projectName, packageManager, workspace, hasChangeset, hasTurbo, gitBranch, buildSteps);
}
...
function generateTsConfig(
    ...
    const pmCommand = packageManager === 'pnpm' ? 'pnpm' : packageManager === 'yarn' ? 'yarn' : 'npm';
...
${hasChangeset ? `  // Changeset 配置（自动检测：检测到 .changeset 目录）
  // changeset: {
...
${buildSteps.map(step => {
  ...
  //   ],
${hasTurbo ? `  //   useTurbo: true, // 自动检测：检测到 turbo.json
  //   turboTasks: ['build'], // 自动检测：从 turbo.json 读取` : ''}
  // },

${workspace.type ? `  // Workspace 配置（自动检测：${workspace.type} workspace）
  // workspace: {
  //   enabled: true, // 自动检测：true
...
${hasChangeset ? `  // Changeset 配置（自动检测）
  // changeset: {
